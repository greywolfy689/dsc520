...
setwd("/Desktop/Bellevue Degree/Book List/520 Class d/Github Repository/dsc520")
...
## `height` vs. `earn`
ggplot(heights_df, aes(x=height, y=earn)) + geom_smooth(method = "lm") + geom_point()
## `age` vs. `earn`
ggplot(heights_df, aes(x=age, y=earn)) + geom_smooth(method = "lm") + geom_point()
## `ed` vs. `earn`
ggplot(heights_df, aes(x=ed, y=earn)) + geom_smooth(method="lm") + geom_point()
...
ggplot(heights_df, aes(x=earn)) + geom_density(stat = "density")
